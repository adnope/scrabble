  ...
  class Text {
      public:
        Text(const string& s, int x, int y, SDL_Renderer* renderer,
            SDL_Color color = white)
            : s(s), x(x), y(y), texture(nullptr), color(color) {
          SDL_Surface* surface =
              TTF_RenderUTF8_Blended(font_Jersey15_Regular, s.c_str(), color);
          if (surface) {
  ...
  public:
    ...
    Button_Text(const string& s, int x, int y, SDL_Renderer* renderer,SDL_Color color = white,bool press=false)
        : s(s), x(x), y(y), texture(nullptr), color(color) {
      SDL_Surface* surface =
          TTF_RenderUTF8_Blended(font_Jersey15_Regular, s.c_str(), color);
      if (surface) {
  ...
  public:
    ...
    Button_Image(string imagePath, int x, int y, SDL_Renderer* renderer,bool press = false)
        : path(imagePath), x(x), y(y), texture(nullptr) {
      loadImage(renderer);
    }
