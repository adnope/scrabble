...
class Bot {
 ...
 private:
  static std::vector<Board::Move> GenerateAllMoves(const std::vector<Tile>& rack,
                                            Board& board);
  static bool IsAnchor(int row, int col, Board& board);
  static void GenerateMoveFromAnchor(int row, int col, const std::vector<Tile>& rack,
                              bool horizontal, std::vector<Board::Move>& moves,
                              Board& board);
  static bool IsValidMove(const Board::Move& move, Board& board);

  // Generate all subsets of tiles from the rack - tiles on hand
  static std::vector<std::vector<Tile>> TilesSubsets(const std::vector<Tile>& rack);
  ...
  static std::vector<std::vector<Tile>> GeneratePermutations(
   ...
   static Board::Move CreateMove(const std::vector<Tile>& tiles, int row, int col,
