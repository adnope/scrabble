...
class Bot {
 ...
 private:
  ...
  static bool VerifyCrossWords(char c, Board& board, Lexicon& lexicon,
                               const Board::Move& move);
  static void GenerateWords(Node* node, MoveGenState& state, Board& board,
                            Lexicon& lexicon, std::vector<Board::Move>& moves);
  std::vector<Board::Move> moves;
  // ProcessOccupiedCell
  // ProcessEmptyCell
  // Gọi cả 2 hàm xử lý NormalTileCase (không blank tile trên tay),
  // BlankTileCase(có 1-2 blank tile trên tay)

  // 1 dãy có nghĩa, nối thêm vào dãy này
  //  sử dụng GetWordFromPos lấy từ -> truyền cho tileData trước khi gọi các hàm
  //  con
  static void ProcessOccupiedCell(Node* node, MoveGenState& state, Board& board,
                                  Lexicon& lexicon,
                                  std::vector<Board::Move>& moves);

  // Điền tự do, có điểm neo
  static void ProcessEmptyCell(Node* node, MoveGenState& state, Board& board,
                               Lexicon& lexicon,
                               std::vector<Board::Move>& moves);
  ...
  // With blank tile
  // đẩy các tile '?' xuống cuối cùng , sử dụng NormalTileCase() cho các chữ
  // thật ->sau đó truy vấn node cho các '?'
  // -:> giảm truy vấn tree
